...
#include <Thread.h>
#include <SyncT.h>
#include <Runnable.h> // For HandleI w/ gcc
#include <HandleI.h>
...
//
void
JTCThreadGroup::create_group_snapshot(JTCThreadGroupSet& set) const
{
...
//
void
JTCThreadGroup::add(const JTCThreadGroupHandle& g)
{
    ...
    else if(m_ngroups == m_groups_capacity)
    {
        ...
        // safe.
        //
	int new_groups_length = m_ngroups << 1;
...
//
void
JTCThreadGroup::remove(const JTCThreadGroupHandle& g)
{
    JTCSyncT<JTCMonitor> sync(*this);
    if(m_destroyed)
	return;
...
//
void
JTCThreadGroup::add(const JTCThreadHandle& t)
{
    JTCSyncT<JTCMonitor> sync(*this);
    if (m_destroyed)
    {
    ...
    else if (m_nthreads == m_threads_capacity)
    {
        ...
        //
	int new_thread_length = m_nthreads << 1;
...
//
void
JTCThreadGroup::remove(const JTCThreadHandle& t)
{
    JTCSyncT<JTCMonitor> sync(*this);
    if(m_destroyed)
	return;
...
//
int
JTCThreadGroup::enumerate(JTCThreadHandle* list, int len, 
                          int n, bool recurse) const
{
...
//
void
JTCThreadGroup::init(const JTCThreadGroupHandle& parent, const char* name)
{
    //
...
JTCThreadGroup::JTCThreadGroup(const JTCThreadGroupHandle& parent,
                               const char* name)
{
    init(parent, name);
}
...
JTCThreadGroup::uncaughtException(JTCThreadHandle t)
{
    ...
    {
#ifndef HAVE_JTC_NO_IOSTREAM
	cerr << "JTCThreadGroup unknown exception" << endl;
#endif // !HAVE_JTC_NO_IOSTREAM
    }
}
...
//
bool
JTCThreadGroup::parentOf(const JTCThreadGroupHandle& handle)
{
    for(JTCThreadGroup* g = handle.get(); g != 0; g = g -> m_parent.get())
    {
	if(g == this)
	    return true;
    }
...
//
int
JTCThreadGroup::enumerate(JTCThreadGroupHandle* list, 
                          int len, bool recurse) const
{
    return enumerate(list, len, 0, recurse);
}
