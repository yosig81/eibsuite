...
//
// This solves the problems associated with using optimaiztions 
// on PA-RISC HP platforms.  The initial thread stack size is 64K 
// and can cause bus errors in some applications when optimizations 
// are turned on.  Increase it here to match the stack size on the 
// HP Itanium platform (256K).
// 
#if defined(__hppa)
static size_t lsd_initial_stack_size = 262144;
#else
      ...
      m_is_adopted(!main)
{
    ...
    //
    // For adopted threads we daemonize the thread group, otherwise there
    // is a resource leak, as the thread group is never destroyed.
    //
    if (m_is_adopted)
    {
...
#   elif defined(__APPLE__) || defined(__CYGWIN__) || defined(__FreeBSD__)
    //
    // REVISIT: bfoucher, need to find out thread priorities
    //
    0,
    0,
    0
...
//
void
JTCThread::setPriority(int new_priority)
{
    JTCSyncT<JTCMutex> guard(m_mutex);
...
#   endif
    ...
    // 
    //Yield();
    // This method has been deprecated in Win32 and was a hold-over from
    // the old cooperative multitasking Windows.  Rather than fix it,
    // Microsoft instead decided to make Sleep(0) do the same.  
    //
    Sleep(0);
#endif
}
...
#if defined(HAVE_POSIX_THREADS)
void
JTCThread::setAttrHook(JTCAttrHook attrHook, JTCAttrHook* oldHook)
{
    if (oldHook != 0)
...
void
JTCThread::setRunHook(JTCRunHook runHook, JTCRunHook* oldHook)
{
    if (oldHook != 0)
	*oldHook = sm_run_hook;
...
void
JTCThread::setStartHook(JTCStartHook startHook, JTCStartHook* oldHook)
{
    if (oldHook != 0)
...
JTCInitialize::initialize()
{
    ...
    m_main_thread -> setName("Main-Thread");
}
...
JTCAdoptCurrentThread::JTCAdoptCurrentThread()
{
	...
	m_adopted_thread = new JTCThread(JTCThreadId::self(), false);
        m_adopted_thread -> setName("AdoptedUserThread");
    }
}
...
#if defined(__CYGWIN__)
#include <windows.h>
void cygSleep(unsigned long millis)
{
    Sleep(millis);
}
#endif
